...
不是「完整实现 CommonMark」—— 是**写作常用的子集 + 明确的规范化**。
逐项实测见 `verify-md-syntax.mjs`，52 项。

**完整支持，往返一致：**

标题 H1–H3、段落、引用、无序 / 有序 / 任务列表与嵌套、代码块（带语言高亮）、
GFM 表格、分隔线、链接、图片（可带宽度）、视频、粗体 / 斜体 / 删除线 / 行内代码 / 高亮、
硬换行、双链 `[[ ]]`。
...
**会降级：** `####` 到 `######` 会被压成三级标题 —— 编辑器只出到 H3。

**当普通文字保留，不渲染：** Setext 标题、四空格缩进代码块、脚注、定义列表、
HTML 标签、LaTeX 公式、单波浪线删除线。**内容不会丢**，只是不按那种语法渲染。

**唯一会丢的：** 开头的 YAML frontmatter 会被剥掉。那是元数据不是正文，刻意为之。
...
```powershell
node test-core.mjs          # Markdown 序列化 / 文件名安全化 / 表格往返（18 项断言）
